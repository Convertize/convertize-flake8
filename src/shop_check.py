...
class ShopVisitor(ast.NodeVisitor):
    NODE_WINDOW_SIZE = 4
    ...
    def visit(self, node):
        ...
        super(ShopVisitor, self).visit(node)
        self.node_stack.pop()
...
class ShopCheck(object):
    name = "convertize-flake8"
    version = __version__

    def __init__(self, tree=None, filename=None, lines=None, visitor=ShopVisitor):
        self.tree = tree
        self.filename = filename
        self.lines = lines
...
Error = partial(partial, error, message=u"", type=ShopCheck, vars=())
...
# B317 = Error(message=u"B317: Use ``from shop.utils import json`` instead.")
